...
const SYSTEM_PROMPT = `אתה מאחד ומסנכרן קטלוג שירותים עבור Vilo Marketplace.
...
- סנכרון מחירים: שלב את המחירים מהטקסט החופשי לתוך השירות המתאים. מחירון לפי מספר משתתפים = מדרגות תמחור: צור שורה לכל מדרגה עם capacity_min=capacity_max=מספר המשתתפים ו-price_ils=המחיר, pricing_unit='group', price_type='fixed', על השירות הנכון (לדוגמה סדנת הקוקטיילים). אם יש מחיר-לאדם לאחר הנחה — שורה עם pricing_unit='person'.
- שירותים שמופיעים רק בטקסט (לדוגמה "בר לאירועים" עם מינימום מחיר/כמות) — צור עבורם שורה אם אינם קיימים כבר.
- הערות מחיר חשובות (כולל/לא כולל מע"מ, עלות נסיעות, תוספות) — שמור ב-supplier_notes של השורה הרלוונטית.
- פריטי תפריט בודדים (קוקטיילים ספציפיים כמו Mojito/Cosmopolitan) אינם שירותים נפרדים למכירה — סכם אותם לתוך התיאור (service_description) של שירות הבר/הסדנה במקום שורות נפרדות.
