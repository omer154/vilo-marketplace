-- ── Migration 005 — return supplier_logo_url from search ───────────────
-- Phase 2d shipped logo upload to the admin, but the marketplace cards
-- and modal never received the URL because search_services only
-- returned supplier_name (not the logo_url). Same fix as 003: add the
-- column to the RPC's RETURNS list and SELECT it from sup.
